...
class MainActivity : AppCompatActivity(), EasyPermissions.PermissionCallbacks,
    ...
    private val locationPermission = 124
        ...
        locationCallback = object : LocationCallback() {
            override fun onLocationResult(locationResult : LocationResult) {
                ...
                if (!isDone) {
                    ...
                    binding.tvCurrentSpeed.text = speedToInt.toString()
    ...
    private fun calcSpeed(speed: Int) {
        if (speed >= 10) {
            ...
            if (speedDownStartTime != 0L) {
                var speedDownTime = speedDownEndTime - speedDownStartTime
                binding.thirtyToTenId.text = (speedDownTime / 1000).toString()
                speedDownTime = 0L
            }
        } else if (speed >= 30) {
            if (speedUpStartTime != 0L) {
                speedUpEndTime = System.currentTimeMillis()
                val speedUpTime = speedUpEndTime
                binding.tenToThirty.text = (speedUpTime / 1000).toString()
                speedUpStartTime = 0L
            }
    ...
    private fun askForLocationPermission() {
        if (hasLocationPermission()) {
            ...
            EasyPermissions.requestPermissions(
                this, "need permissions to find your" +
                        " location and calculate the speed ",
